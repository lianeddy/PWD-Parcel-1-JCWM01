...
function UserTransaksi() {
  ...
  const user = async (status) => {
    ...
    else url = `http://localhost:3302/order/transaction?id=${id}`;
  ...
  function onFileUpload(event) {
    ...
    formData.append("orderDetailId", current_order_detail_id);
    ...
    Axios.post("http://localhost:3302/order/upload-payment", formData).then(
      (res) => {
            ...
            <div>
              <Card>
                <CardBody>
                  ...
                  <Row>
                    <div className="col-6">{transaction.products}</div>
                  </Row>
                  {/* if condition react */}
                  {transaction.status == 0 && (
                    <Button
                      color="danger"
                      onClick={() => toggle1(transaction.order_detail_id)}
                    >
                      Payment Proof
                    </Button>
                  )}
